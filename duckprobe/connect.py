"""Native cognis-connect emit for duckprobe — forward findings to any platform.

Maps duckprobe's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    duckprobe ... --format json | duckprobe-emit --to stix
    duckprobe-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('check', '') + ' - ' + rec.get('source', '')
        out['severity'] = {'pass': 'info', 'fail': 'high', 'warn': 'medium'}.get(rec.get('outcome'), 'info')
        out['type'] = 'finding'
        out['description'] = rec.get('obs', '')
        out['tags'] = [rec.get('source', ''), rec.get('engine', '')]
